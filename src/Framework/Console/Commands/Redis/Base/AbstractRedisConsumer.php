...
abstract class AbstractRedisConsumer extends Command
{
     ...
     *
     * @param string|null $brokerName Broker name (default: 'redis')
     * @return RedisBroker
     * @throws \RuntimeException If broker not found or not a Redis broker
     */
    protected function getBroker(?string $brokerName = null): RedisBroker
    {
        ...
        if (!$broker) {
            throw new \RuntimeException(
                "Redis broker '{$brokerName}' not found in configuration. " .
                    "Make sure broker is configured in config/realtime.php"
            );
        }

        if (!$broker instanceof RedisBroker) {
            throw new \RuntimeException("Broker '{$brokerName}' is not a Redis broker");
        }
    ...
    {
        return $this->option('broker', 'redis');
    }
    ...
    protected function displayHeader(string $consumerType, array $options = []): void
    {
        ...
        $this->newLine();
        $this->info("Redis Consumer: {$consumerType}");
        $this->line('=', 60);
    ...
    {
        $this->errors++;
        $message = "Error processing Redis message: {$exception->getMessage()}";
    ...
    /**
     * Render a styled Redis log line.
     *
     ...
     * @return void
     */
    protected function logRedisEvent(string $label, string $message, string $style = 'info'): void
    {
